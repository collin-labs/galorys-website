        ...
        if (response.ok) {
          ...
          if (data.partner) {
            setFormData({
              ...
              active: data.partner.active ?? true,
            })
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        alert(data.error || 'Erro ao atualizar parceiro')
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">Editar Parceiro</h1>
            <p className="text-sm text-muted-foreground">Editando: {formData.name}</p>
          </div>
        </div>
      </div>
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      ...
                      {formData.logo ? 'Trocar Imagem' : 'Escolher Imagem'}
                    </>
                  )}
                </Button>
