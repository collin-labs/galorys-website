...
async function fetchAllRobloxData() {
  ...
  // 2. NOVO: Tentar buscar jogos da tabela PlayableGame primeiro
  try {
    const dbGames = await prisma.playableGame.findMany({
      where: { 
        platform: "roblox",
        active: true 
      },
    ...
    if (dbGames.length > 0) {
      console.log(`GamesStats: Encontrados ${dbGames.length} jogos Roblox no banco (PlayableGame)`)

      // Buscar stats de cada jogo cadastrado
      for (const dbGame of dbGames) {
        try {
          let universeId = dbGame.universeId || dbGame.externalId
          
          // Se o externalId parece ser um Place ID (número grande), converter para Universe ID
          if (!dbGame.universeId && dbGame.externalId.length > 10) {
            const universeResponse = await fetch(
              `https://apis.roblox.com/universes/v1/places/${dbGame.externalId}/universe`,
              { next: { revalidate: 30 } }
            )
            if (universeResponse.ok) {
              ...
              // Salvar universeId para próximas consultas
              if (universeId) {
                await prisma.playableGame.update({
                  where: { id: dbGame.id },
                  data: { universeId }
                }).catch(() => {})
              }
          ...
          games.push({
            id: dbGame.externalId,
            universeId: universeId,
            name: gameStats?.name || dbGame.name,
            ...
            url: dbGame.gameUrl || `https://www.roblox.com/games/${dbGame.externalId}`,
            // Campos extras do banco
            slug: dbGame.slug,
            featured: dbGame.featured,
            order: dbGame.order,
            instagram: dbGame.instagram,
            discord: dbGame.discordInvite,
          } as any)

          console.log(`GamesStats Roblox: ${dbGame.name} = ${playing} jogadores`)
        } catch (e) {
          console.log(`GamesStats: Erro ao buscar ${dbGame.name}`)
        }
      }

      // Se conseguiu buscar do banco, retorna
      if (games.length > 0) {
        return {
          group: groupData ? {
    ...
    console.log("GamesStats: Tabela PlayableGame não disponível, usando fallback do grupo")
  }

  // 3. FALLBACK: Buscar lista de jogos do grupo automaticamente
  try {
    ...
    if (gamesListResponse.ok) {
      ...
      console.log(`GamesStats: Encontrados ${gamesFromGroup.length} jogos Roblox no grupo (fallback)`)

      // Buscar stats de cada jogo
...
async function fetchAllFivemData() {
  ...
  // 1. NOVO: Tentar buscar servidores da tabela PlayableGame primeiro
  try {
    const dbServers = await prisma.playableGame.findMany({
      where: { 
        platform: "fivem",
        active: true 
      },
    ...
    if (dbServers.length > 0) {
      console.log(`GamesStats: Encontrados ${dbServers.length} servidores FiveM no banco (PlayableGame)`)

      for (const dbServer of dbServers) {
        try {
          const response = await fetch(
            `https://servers-frontend.fivem.net/api/servers/single/${dbServer.externalId}`,
            {
              headers: {
                'Accept': 'application/json',
                'User-Agent': 'Galorys-Website/1.0'
          ...
          if (response.ok) {
            ...
            foundCodes.add(dbServer.externalId)

            servers.push({
              code: dbServer.externalId,
              name: dbServer.name,
              game: dbServer.slug,
              players,
              maxPlayers: data?.Data?.sv_maxclients || 0,
              online: players > 0,
              hostname: data?.Data?.hostname?.replace(/\^[0-9]/g, '') || dbServer.name,
              url: dbServer.gameUrl || `https://cfx.re/join/${dbServer.externalId}`,
              instagram: dbServer.instagram || undefined,
              discord: dbServer.discordInvite || undefined,
              video: dbServer.videoPath || undefined,
              // Campos extras do banco
              slug: dbServer.slug,
              featured: dbServer.featured,
              order: dbServer.order,
            } as any)

            console.log(`GamesStats FiveM: ${dbServer.name} = ${players} jogadores`)
          }
        } catch (e) {
          console.log(`GamesStats: Erro ao buscar servidor ${dbServer.name}`)
        }
      }
    ...
    console.log("GamesStats: Tabela PlayableGame não disponível para FiveM, usando fallback")
  }

  // 2. FALLBACK: Busca automática por tags
